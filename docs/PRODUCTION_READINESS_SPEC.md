...
| RLS: the opposite direction of BUG-001 — several tables also carried a still-permissive `anon`/`public` policy alongside the correct `authenticated` one, so the anon key (shipped in the client bundle) had full read/write access via PostgREST regardless of login, bypassing every app-layer check | ✅ **Resolved and Production-verified (this project's own record of BUG-002).** Closed table-by-table via dedicated `DROP POLICY IF EXISTS ... anon ...` migrations after each table's server-side code was confirmed to thread a real authenticated client end-to-end (no live path left depending on the anon fallback). `activity_logs` was the final table in this batch — migration `2026082501_bug002_activity_logs_rls_lockdown.sql`, applied and independently re-verified live against Production: `"Allow full access to anon"` absent, `"Allow full access to authenticated"` present and unchanged, no other table or business data touched. Unlike the BUG-001 row above, this fix **was** live-verified after applying, not just applied. |
