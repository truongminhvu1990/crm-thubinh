 ...
 */
let nextListProductsResult: { data: { batch_id: string; status: string }[] | null; error: unknown } = {
  data: [],
  error: null,
};
...
test("computeBatchCounts — Case 3: all Returned", async () => {
  const { computeBatchCounts } = await import("./productBatch.service");
  const rows = [{ status: "Returned" }, { status: "Returned" }];
...
test("computeBatchCounts — Case 4: all Remaining", async () => {
  const { computeBatchCounts } = await import("./productBatch.service");
  const rows = [{ status: "Active" }, { status: "Paused" }, { status: "Discontinued" }];
...
test("computeBatchCounts — Case 5: mixed statuses", async () => {
  const { computeBatchCounts } = await import("./productBatch.service");
  const rows = [{ status: "Sold" }, { status: "Returned" }, { status: "Active" }, { status: "Reserved" }, { status: "Paused" }];
...
test("Case 6: getBatchListStats — multiple batch_ids in one dataset never mix KPI", async () => {
  nextListProductsResult = {
    data: [
      ...
      { batch_id: "batch-A", status: "Active" },
      { batch_id: "batch-B", status: "Returned" },
      { batch_id: "batch-B", status: "Returned" },
...
test("Case 8: getBatchListStats — exactly one supabase.from() call regardless of how many Lots/products (no N+1)", async () => {
  nextListProductsResult = {
    data: Array.from({ length: 50 }, (_, i) => ({ batch_id: `batch-${i % 5}`, status: i % 2 === 0 ? "Sold" : "Active" })),
    error: null,
  };
...
test("Case 7: getBatchStats and getBatchListStats agree on the same fixture", async () => {
  const fixtureRows = [
    ...
    { batch_id: "batch-X", status: "Sold" },
    { batch_id: "batch-X", status: "Returned" },
    { batch_id: "batch-X", status: "Active" },
  ...
  const detailCounts = computeBatchCounts(fixtureRows.map((r) => ({ status: r.status })));
