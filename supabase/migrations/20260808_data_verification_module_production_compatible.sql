-- Data Verification Center (Sprint v2.3.0) - Production-compatible version.
--
...
-- position, with entry_source/created_by/updated_by/updated_at appended
-- at the end - additive only, BR-001's expression is untouched.
--
...
SELECT
  ...
  CASE
    ...
    ELSE false
  END AS is_duplicate,
  -- BR-001 (docs/ORDERS_SPEC.md "Business Rule Lock", LOCKED) - unchanged
  -- from 20260807_sales_ledger_revenue_recognition.sql.
...
-- Column order sanity check - expect is_revenue_recognized at position 20,
-- entry_source/created_by/updated_by/updated_at at 21-24:
